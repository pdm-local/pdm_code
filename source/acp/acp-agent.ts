...
export class AcpAgent implements Agent {
	...
	async loadSession(params: LoadSessionRequest): Promise<LoadSessionResponse> {
		...
		return {
			_meta: {
				'pdm/artifacts': await listSessionArtifacts(params.sessionId),
			},
			modes: this.buildModeState(session),
			configOptions: await this.buildConfigOptions(),
		};
		...
		if (trimmedUserText.startsWith('/')) {
			...
			if (!commandName.includes('/')) {
				...
				if (command) {
					...
					if (commandName === 'copy') {
						const msg =
							'`/copy` is handled by the chat view. Type it in the PDM Code chat input (or press Ctrl+Alt+Shift+C / Cmd+Alt+Shift+C for the last code block).';
						return sendBuiltinReply(msg);
					}
					...
					if (commandName === 'settings') {
						const msg =
							'Use the Settings tab in the PDM Code sidebar (the gear icon in the view title bar, or `PDM Code: Settings` in the Command Palette).';
						return sendBuiltinReply(msg);
					}
					...
					if (
						...
						const msg = `The \`/${commandName}\` command is only available in the interactive CLI (\`pdm\` in a terminal). It is not supported in the VS Code GUI.`;
						return sendBuiltinReply(msg);
					}

...
import {getLogger, isLevelEnabled} from '@/utils/logging';
...
function addTypedMessage(
	...
	// Only instrument when the debug line will actually be emitted. Both
	// startMetrics and endMetrics call process.memoryUsage(), which reads
	// /proc/self on Linux - two syscall-heavy reads per queued message, to
	// report how long it took to construct a React element.
	const shouldTrackMetrics = isLevelEnabled('debug');
	const metrics = shouldTrackMetrics ? startMetrics() : null;

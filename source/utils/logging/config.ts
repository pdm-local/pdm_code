 ...
 * - Linux: ~/.local/state/pdm/logs (XDG_STATE_HOME)
 * - Windows: %LOCALAPPDATA%/pdm/logs
 */
export function getDefaultLogDirectory(): string {
	if (process.env.PDM_LOG_DIR) {
		return process.env.PDM_LOG_DIR;
	}
	...
	switch (platform()) {
		...
		case 'darwin':
			return join(homedir(), 'Library', 'Logs', 'pdm');
		default: // linux
			return join(
...
function createDevelopmentConfig(): EnhancedLoggerConfig {
	return {
		level: (process.env.PDM_LOG_LEVEL as LogLevel) || 'debug',
 ...
 * getEnvironmentConfig() (PDM_LOG_LEVEL opt-in), not by this function.
...
function createProductionConfig(): EnhancedLoggerConfig {
	...
	const disableFileLogging = process.env.PDM_LOG_DISABLE_FILE === 'true';

	// File log level defaults to 'info' for useful diagnostics
	// Can be overridden with PDM_LOG_LEVEL env var
	const fileLogLevel =
		(process.env.PDM_LOG_LEVEL as LogLevel) ||
 ...
 * This gives users a clean experience. Developers working on pdm itself should
...
function getEnvironmentConfig(): EnhancedLoggerConfig {
	...
	// Pretty/verbose console output is explicit opt-in via PDM_LOG_LEVEL,
	// not implicit based on NODE_ENV. This avoids polluting user output in production.
	if (process.env.PDM_LOG_LEVEL) {

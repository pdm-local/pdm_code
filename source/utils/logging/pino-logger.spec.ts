...
// Test utilities
const testLogDir = join(tmpdir(), `pdm-pino-test-${Date.now()}`);
...
test('logger handles correlation context', async t => {
	...
	const originalCorrelation = process.env.PDM_CORRELATION_ENABLED;
	process.env.PDM_CORRELATION_ENABLED = 'true';
	...
	if (originalCorrelation === undefined) {
		delete process.env.PDM_CORRELATION_ENABLED;
	} else {
		process.env.PDM_CORRELATION_ENABLED = originalCorrelation;
	}
});
...
test('logger handles correlation when disabled', async t => {
	...
	const originalCorrelation = process.env.PDM_CORRELATION_ENABLED;
	process.env.PDM_CORRELATION_ENABLED = 'false';

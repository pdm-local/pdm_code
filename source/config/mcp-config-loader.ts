...
/**
 * Load global MCP configuration from ~/.config/pdm/.mcp.json
 */
...
function loadEnvMCPConfigs(): MCPServerWithSource[] {
	...
	if (!rawData && process.env.PDM_MCPSERVERS_FILE) {
		if (existsSync(process.env.PDM_MCPSERVERS_FILE)) {
			try {
				rawData = readFileSync(process.env.PDM_MCPSERVERS_FILE, 'utf-8');
			} catch (error) {
				logError(
					`Failed to read PDM_MCPSERVERS_FILE at ${process.env.PDM_MCPSERVERS_FILE}: ${String(error)}`,
				);
			}
		}
	}
...
function loadProjectProviderConfigs(): ProviderConfig[] {
	...
	if (existsSync(configPath)) {
		try {
			...
			if (config.pdm && Array.isArray(config.pdm.providers)) {
				// Apply environment variable substitution
				const processedProviders = substituteEnvVars(config.pdm.providers);
...
function loadEnvProviderConfigs(): ProviderConfig[] {
	let rawData = process.env.PDM_PROVIDERS;

	if (!rawData && process.env.PDM_PROVIDERS_FILE) {
		if (existsSync(process.env.PDM_PROVIDERS_FILE)) {
			try {
				rawData = readFileSync(process.env.PDM_PROVIDERS_FILE, 'utf-8');
			} catch (error) {
				logError(
					`Failed to read PDM_PROVIDERS_FILE at ${process.env.PDM_PROVIDERS_FILE}: ${String(error)}`,
				);
			}
		}
	}
	...
	try {
		...
		} else if (config.pdm && Array.isArray(config.pdm.providers)) {
			return substituteEnvVars(config.pdm.providers);
		} else if (Array.isArray(config.providers)) {

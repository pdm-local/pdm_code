...
export function getClosestConfigFile(fileName: string): string {
	try {
		...
		// If PDM_CONFIG_DIR is explicitly set, skip cwd and home checks
		// and use only the config directory (important for tests and explicit overrides)
		const isExplicitConfigDir = Boolean(process.env.PDM_CONFIG_DIR);
...
function loadAutoCompactConfig(): AutoCompactConfig {
	...
	return (
		loadHierarchicalConfig('agents.config.json', 'auto-compact', config => {
			const autoCompact = config.pdm?.autoCompact;
...
function loadSessionConfig(): AppConfig['sessions'] {
	...
	return (
		loadHierarchicalConfig('pdm-preferences.json', 'session', config => {
			const sessions = config.pdm?.sessions;
...
// wedged model in CI can't run unbounded. Override via the PDM_MAX_TURNS
// env var or `pdm.headless.maxTurns` in agents.config.json.
...
function loadHeadlessConfig(): AppConfig['headless'] {
	...
	const envValue = process.env['PDM_MAX_TURNS'];
	...
	return (
		loadHierarchicalConfig('agents.config.json', 'headless', config => {
			const headless = config.pdm?.headless;
...
// Load agent-loop retry limits from `pdm.retries` in agents.config.json.
...
function loadRetryLimitsConfig(): RetryLimitsConfig {
	...
	return (
		loadHierarchicalConfig('agents.config.json', 'retries', config => {
			const retries = config.pdm?.retries;
...
function loadPasteConfig(): PasteConfig {
	...
	return (
		loadHierarchicalConfig('pdm-preferences.json', 'paste', config => {
			const paste = config.pdm?.paste;
...
function loadPdmToolsConfig(): AppConfig['pdmTools'] {
	return (
		loadHierarchicalConfig('agents.config.json', 'pdmTools', config => {
			const pdmTools = config.pdm?.pdmTools;
			if (pdmTools && typeof pdmTools === 'object') {
				return substituteEnvVars(pdmTools);
			}
			return null;
		}) ?? undefined
	);
...
function loadDisabledToolsConfig(): string[] | undefined {
	return (
		loadHierarchicalConfig('agents.config.json', 'disabledTools', config => {
			const disabledTools = config.pdm?.disabledTools;
...
function loadModeProvidersConfig(
	...
	return (
		loadHierarchicalConfig('agents.config.json', 'modeProviders', config => {
			const modeProviders = config.pdm?.modeProviders;
			if (
				!modeProviders ||
...
function loadAppConfig(): AppConfig {
	...
	// Load pdm tools configuration
	const pdmTools = loadPdmToolsConfig();

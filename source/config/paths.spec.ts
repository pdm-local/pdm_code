...
// These tests intentionally lock in the public contract for PDM Code's
...
test.serial('getAppDataPath uses PDM_DATA_DIR override verbatim', t => {
	testPathGetter(
		t,
		'linux',
		{
			PDM_DATA_DIR: '/custom/data',
			APPDATA: 'C:/Ignored',
...
test.serial('getAppDataPath darwin default path is stable', t => {
	testPathGetter(
		...
		{HOME: '/Users/test'},
		getAppDataPath,
		path.join('/Users/test', 'Library', 'Application Support', 'pdm'),
	);
});
...
test.serial('getAppDataPath win32 uses APPDATA when set', t => {
	testPathGetter(
		...
		getAppDataPath,
		path.join('C:', 'Users', 'test', 'AppData', 'Roaming', 'pdm'),
	);
});
...
test.serial('getAppDataPath linux falls back to ~/.local/share', t => {
	testPathGetter(
		...
		getAppDataPath,
		path.join('/home/test', '.local', 'share', 'pdm'),
	);
});
...
test.serial('getConfigPath uses PDM_CONFIG_DIR override verbatim', t => {
	testPathGetter(
		t,
		'linux',
		{
			PDM_CONFIG_DIR: '/custom/config',
...
test.serial('getConfigPath darwin default path is stable', t => {
	testPathGetter(
		t,
		'darwin',
		{PDM_CONFIG_DIR: undefined, HOME: '/Users/test'},
		getConfigPath,
		path.join('/Users/test', 'Library', 'Preferences', 'pdm'),
	);
});
...
test.serial('getConfigPath linux falls back to ~/.config', t => {
	testPathGetter(
		...
		getConfigPath,
		path.join('/home/test', '.config', 'pdm'),
	);
});

...
test('loadEnvMCPConfigs - loads from PDM_MCPSERVERS environment variable', t => {
    const originalValue = process.env.PDM_MCPSERVERS;
    ...
    try {
        ...
        process.env.PDM_MCPSERVERS = JSON.stringify(config);
        ...
        if (originalValue !== undefined) {
            process.env.PDM_MCPSERVERS = originalValue;
        } else {
...
test('loadEnvMCPConfigs - loads from PDM_MCPSERVERS_FILE', t => {
    const testDir = t.context.testDir as string;
    const originalValue = process.env.PDM_MCPSERVERS;
    const originalFileValue = process.env.PDM_MCPSERVERS_FILE;
    ...
    try {
        ...
        process.env.PDM_MCPSERVERS_FILE = filePath;
        delete process.env.PDM_MCPSERVERS; // Ensure direct var doesn't interfere
        ...
        if (originalFileValue !== undefined) {
            process.env.PDM_MCPSERVERS_FILE = originalFileValue;
        } else {
...
test('loadEnvMCPConfigs - handles invalid JSON gracefully', t => {
    ...
    try {
        process.env.PDM_MCPSERVERS = 'invalid json {';
...
test('loadEnvProviderConfigs - loads from PDM_PROVIDERS environment variable', t => {
    const originalValue = process.env.PDM_PROVIDERS;
    ...
    try {
        ...
        process.env.PDM_PROVIDERS = JSON.stringify(providers);
        ...
        if (originalValue !== undefined) {
            process.env.PDM_PROVIDERS = originalValue;
        } else {
...
test('loadEnvProviderConfigs - supports pdm wrapper format', t => {
    const originalValue = process.env.PDM_PROVIDERS;
    ...
    try {
        ...
        process.env.PDM_PROVIDERS = JSON.stringify(config);
...
test('hierarchical precedence - env overrides project and global', t => {
    const testDir = t.context.testDir as string;
    const originalMcpServers = process.env.PDM_MCPSERVERS;
    const originalConfigDir = process.env.PDM_CONFIG_DIR;
    ...
    try {
        ...
        process.env.PDM_MCPSERVERS = JSON.stringify(envConfig);
        ...
        if (originalMcpServers !== undefined) {
            process.env.PDM_MCPSERVERS = originalMcpServers;
        } else {
            delete process.env.PDM_MCPSERVERS;
        }
        if (originalConfigDir !== undefined) {
...
test('hierarchical precedence - PDM_PROVIDERS overrides all', t => {
    const testDir = t.context.testDir as string;
    const originalProviders = process.env.PDM_PROVIDERS;
    const originalConfigDir = process.env.PDM_CONFIG_DIR;
    const originalNodeEnv = process.env.NODE_ENV;
    ...
    try {
        ...
        process.env.PDM_PROVIDERS = JSON.stringify(envConfig);
        ...
        if (originalProviders !== undefined) {
            process.env.PDM_PROVIDERS = originalProviders;
        } else {
            delete process.env.PDM_PROVIDERS;
        }
        if (originalConfigDir !== undefined) {

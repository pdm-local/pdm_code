...
async function probeLocalProvider(
	...
	if (process.env.PDM_DOCTOR_PROBE === '0') {
		return 'skipped';
	}
...
export async function collectDoctorReport(
	...
	const [pdm, providers, lsp, mcp, daemonLock] = await Promise.all([
...
export function Doctor({report}: {report: DoctorReport}) {
	...
	return (
		...
		>
			...
			<SectionTitle>PDM Code</SectionTitle>
			{report.pdm.status === 'ok' ? (
				<Text color={colors.text}>• version {report.pdm.data.version}</Text>
			) : (
				<SectionError message={report.pdm.error} />
			)}

 ...
 *
 * Spawned by `pdm daemon start`. Reads the project root from
 * `PDM_PROJECT_ROOT` (set by the spawn) or falls back to cwd, then
 * stands up the full skill / event pipeline via `startDaemon`.
 *
 * stdio is redirected to `.pdm/daemon.log` by the launcher, so
 * `console.log` / `console.error` calls here end up in the log file
 * `pdm daemon logs` tails.
 */
...
async function main(): Promise<void> {
	const projectRoot = process.env.PDM_PROJECT_ROOT || process.cwd();

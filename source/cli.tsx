...
// Handle `pdm daemon <sub>`, fast path, only loads the daemon
...
if (args[0] === 'daemon') {
	...
	if (!sub || !(valid as readonly string[]).includes(sub)) {
		console.error(
			'Usage: pdm daemon <start|stop|status|logs|install|uninstall>',
		);
...
Examples:
  ...
  pdm --provider ollama --model llama3.1 --context-max 128k
  pdm --mode yolo run "refactor database module"
  pdm --mode plan
  pdm --trust-directory run "analyze src/app.ts"
  pdm --plain run "summarize README.md"
  pdm --plain --json run "summarize README.md" | jq .finalText
  pdm --continue
  pdm --resume last
  pdm --resume
  `);
...
async function main(): Promise<void> {
	...
	if (trustDirectoryRequested && !nonInteractiveMode) {
		console.error(
			'--trust-directory only applies to non-interactive mode (`pdm run ...`); ignoring.',
		);
	}
	...
	if (plainRequested && !nonInteractiveMode) {
		console.error(
			'--plain requires the `run` subcommand in this version. Try: pdm --plain run "..."',
		);
		process.exit(1);
	}
	...
	if (args[0] === 'codex' && args[1] === 'login') {
		...
		// interactive TUI only. Run mode (`pdm run …`) prints a
		// transcript the user needs to keep after exit, the alt screen
		// would discard it when restoring the original buffer.
		// Screen mode: inline (main screen + native scrollback) by DEFAULT, // the terminal's own scrollbar, wheel, and search work there.

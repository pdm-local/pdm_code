...
// These tests exercise the built binary, but `pnpm test:ava` never builds. A
// missing dist used to surface as a bare ENOENT from execFileSync; say what to
// do about it instead.
//
// Note this cannot detect a *stale* dist, if the build is older than the
// source, these tests quietly assert against the previous build. Only running
// `pnpm build` first closes that gap.
test.before(() => {
	if (!existsSync(cliPath)) {
		throw new Error(
			`dist/cli.js not found at ${cliPath}. Run \`pnpm build\` before the CLI integration specs.`,
		);
	}
});

...
test.serial(
	'FileSnapshotService restores a file whose name begins with dots',
	async t => {
		const tempDir = await createTempDir();
		try {
			const service = new FileSnapshotService(tempDir);
			// Regression: the containment check used to be
			// `path.relative(root, abs).startsWith('..')`, which reads a
			// legitimately-named file INSIDE the workspace as an escape, // `path.relative(root, root + '/..config')` is `'..config'`.
			await service.restoreFiles(new Map([['..config', 'kept']]));

			t.is(await fs.readFile(path.join(tempDir, '..config'), 'utf-8'), 'kept');
		} finally {
			await cleanupTempDir(tempDir);
		}
	},

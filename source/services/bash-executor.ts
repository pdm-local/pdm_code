...
export class BashExecutor extends EventEmitter {
	...
	execute(
		...
		const cwdCaptureFile = isWindows
			? undefined
			: join(tmpdir(), `pdm-cwd-${executionId}`);
		const spawnCommand =
		...
		// Progress emits are coalesced. Every chunk used to emit, and each emit
		// drives a setState and a full Ink re-render - a chatty command like a
		// build or a test run produces hundreds of chunks per second. Ink paints
		// at 30fps, so emitting faster than this window is invisible work.
		//
		// Output is still buffered on every chunk; only the notification is
		// throttled, so no data is lost or delayed in the final result.
		let lastEmitAt = 0;
		...
		const emitProgress = () => {
			...
			// Dropped once the execution has completed or been cancelled: a late
			// progress event after 'complete' would report stale state.
		...
		const scheduleProgress = () => {
			...
			if (sinceLast >= BASH_PROGRESS_COALESCE_MS) {
				emitProgress();
				return;
			}
			pendingEmit = setTimeout(
				emitProgress,
				BASH_PROGRESS_COALESCE_MS - sinceLast,
			);
			// Never let a queued repaint hold the process open.
			pendingEmit.unref();
		};
		...
		proc.stdout.on('data', (data: Buffer) => {
			...
			// Leading-edge, so a fast command still shows its output straight
			// away; subsequent chunks inside the window ride one trailing emit.
			scheduleProgress();
		});
		...
		// Heartbeat for commands that run long while producing no output, so the
		// elapsed time keeps moving. Routed through emitProgress so it shares the
		// coalescing window rather than double-emitting alongside output.
		// unref()'d so it never prevents Node from exiting (tests, clean shutdown).

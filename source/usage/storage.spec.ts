...
function createTestDir(): string {
	const testDir = path.join(
		os.tmpdir(),
		`pdm-test-${Date.now()}-${Math.random().toString(36).substring(7)}`,
	);
...
test('migrates usage data from legacy config directory to app data directory', t => {
	...
	// When PDM_CONFIG_DIR is set, getConfigPath() returns it directly.
	// So here we create a fake legacy config dir and point PDM_CONFIG_DIR at it.
	const legacyConfigDir = path.join(os.tmpdir(), 'pdm-legacy-config');
	...
	// Point PDM_CONFIG_DIR to our legacy config dir to simulate pre-change behavior
	process.env.PDM_CONFIG_DIR = legacyConfigDir;
	...
	const appDataDir = path.join(appDataHome, 'pdm');
...
test('migration removes legacy file after successful migration', t => {
	// Create legacy file
	const legacyConfigDir = path.join(os.tmpdir(), 'pdm-legacy-config-2');
	fs.mkdirSync(legacyConfigDir, {recursive: true});
	const legacyFilePath = path.join(legacyConfigDir, 'usage.json');
	const legacyData: UsageData = {
...
test('migration skips when new file already exists', t => {
	...
	const legacyConfigDir = path.join(os.tmpdir(), 'pdm-legacy-config-3');
	fs.mkdirSync(legacyConfigDir, {recursive: true});
	const legacyFilePath = path.join(legacyConfigDir, 'usage.json');
	const legacyData: UsageData = {
...
test('migration handles missing legacy config directory gracefully', t => {
	// Don't create legacy directory, just set PDM_CONFIG_DIR to non-existent path
	process.env.PDM_CONFIG_DIR = path.join(
		os.tmpdir(),
		'pdm-nonexistent',
	);
...
test('migration handles corrupt legacy file gracefully', t => {
	...
	const legacyConfigDir = path.join(os.tmpdir(), 'pdm-legacy-config-4');
	fs.mkdirSync(legacyConfigDir, {recursive: true});
	const legacyFilePath = path.join(legacyConfigDir, 'usage.json');
...
test('migration preserves all session data fields', t => {
	const legacyConfigDir = path.join(os.tmpdir(), 'pdm-legacy-config-5');
	fs.mkdirSync(legacyConfigDir, {recursive: true});
	const legacyFilePath = path.join(legacyConfigDir, 'usage.json');
...
test('readUsageData returns empty data on read error', t => {
	...
	const configDir = path.join(dataHome, 'pdm');

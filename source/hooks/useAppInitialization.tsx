...
export function useAppInitialization({
	...
	const initializeClient = async (
		...
		// blocks surface as warnings in the chat queue, without a box, // matches the existing "no box" convention for inline diagnostics.
	...
	const start = async (
		...
		try {
					...
					if (nonInteractiveMode) {
						...
						addToChatQueue(
							<ErrorMessage
								...
								message="No providers configured. Run pdm interactively to set them up."
								hideBox={true}
							/>,
						);
					} else {
			...
			// In non-interactive mode there's no human to recover via /model, // keep init from blocking on a null client by exiting once the
	...
	useEffect(() => {
		const initializeApp = async () => {
			...
			// === BACKGROUND WORK ===
			// Both run concurrently after the chat is interactive. MCP tools
			// register dynamically as servers connect; LSP diagnostics appear
			// when ready.

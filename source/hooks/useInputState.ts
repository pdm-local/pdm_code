...
export function useInputState() {
	...
	//
	// Bounded: this runs on every keystroke, and each entry retains a full
	// InputState including any paste placeholder contents. Unbounded, a long
	// prompt kept one snapshot per character alive for the life of the session.
	// Dropping the oldest entry past the cap costs only the ability to undo
	// beyond MAX_UNDO_HISTORY steps, which no one reaches by hand.
	const pushToUndoStack = useCallback(
		(newState: InputState) => {
			setUndoStack(prev => {
				const next = [...prev, currentState];
				return next.length > MAX_UNDO_HISTORY
					? next.slice(next.length - MAX_UNDO_HISTORY)
					: next;
			});
	...
	// The text lands at the end of the input rather than at the cursor, // the payload arrives out of band, so the cursor offset TextInput owns

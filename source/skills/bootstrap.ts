 ...
 *   3. BundleLoader loads bundle-form skills under .pdm/skills/.
...
function detectDeprecations(projectRoot: string): string[] {
	...
	const schedulesJson = join(projectRoot, '.pdm', 'schedules.json');
	if (existsSync(schedulesJson)) {
		warnings.push(
			`.pdm/schedules.json is deprecated. Move each entry into the targeted command's frontmatter as a "schedule.cron" subscription, or into a skill bundle's manifest. Run \`pdm daemon start\` to enable scheduled runs.`,
		);
	}
	return warnings;
}

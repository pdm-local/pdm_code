 ...
 * PDM_CONFIG_DIR is set so the loader's personal-layer lookup hits the
...
test.serial(
	...
	async t => {
		await withTempEnv(async ({projectRoot}) => {
			const bundleRoot = join(projectRoot, '.pdm', 'skills');
...
test.serial(
	...
	async t => {
		await withTempEnv(async ({projectRoot, personalRoot}) => {
			...
			await makeBundle(join(projectRoot, '.pdm', 'skills'), 'shared', {

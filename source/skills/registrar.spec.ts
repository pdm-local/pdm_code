...
test('registers a flat command into the CustomCommandLoader', t => {
	...
	const skill = singleFileCommandSkill(
		'weekly-report',
		'/p/.pdm/commands/weekly-report.md',
	);
...
test('registers a subagent with ownerSkill tag into SubagentLoader', async t => {
	...
	const skill: Skill = {
		...
		source: {
			priority: 'project',
			shape: 'single-file',
			rootPath: '/p/.pdm/agents/docs-agent.md',
		},
		subagent: {
			subagent: {
				name: 'docs-agent',
			...
			filePath: '/p/.pdm/agents/docs-agent.md',
		},
	};
	const r = registerSkills([skill], deps);
	t.deepEqual(r.collisions, []);
...
test('command-name collision is reported, the second registration is skipped', t => {
	const deps = makeDeps();
	const a = singleFileCommandSkill('same', '/a/.pdm/commands/same.md');
	const b = singleFileCommandSkill('same', '/b/.pdm/commands/same.md');
	...
	// First one wins
	t.is(deps.commandLoader.getCommand('same')?.path, '/a/.pdm/commands/same.md');
});

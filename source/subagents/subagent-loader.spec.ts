...
test.serial('loads project-level agent without permissionMode', async t => {
	const tempDir = join(tmpdir(), `pdm-test-${Date.now()}`);
...
// ============================================================================
// Gap #7: Project-level agent loading from .pdm/agents/
// ============================================================================

test.serial('loads project-level agents from .pdm/agents/', async t => {
	const tempDir = join(tmpdir(), `pdm-test-${Date.now()}`);
	const agentsDir = join(tempDir, '.pdm', 'agents');
	mkdirSync(agentsDir, {recursive: true});

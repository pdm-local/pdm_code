...
export interface Message {
	 ...
	 * `convertToModelMessages`) so the model is never handed pdm's own
...
export interface PdmCodeToolExport {
	name: string;
	tool: AISDKCoreTool;
...
export interface LLMClient {
	...
	/** Releases held resources (e.g. a keep-alive connection pool). Optional: only clients that own such resources need it. */

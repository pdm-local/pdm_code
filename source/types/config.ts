...
export interface AppConfig {
	...
	// PDM Code-specific tool configurations
	pdmTools?: {
		webSearch?: {
			apiKey?: string;
		};
...
export interface OpenRouterParameters {
	...
	// is more expensive / lower latency. There is no `auto` request value, // OpenRouter only reports `auto`/`default`/`standard` back in the response.
...
export interface UserPreferences {
	...
	/**
	 * Delegate model for `analyze_image`/pasted-image routing when the active
	 * model can't accept image input. Deliberately not a provider entry in
	 * agents.config.json, that would join the fallback rotation other
	 * providers use and could end up serving ordinary coding traffic from a
	 * small vision model.
	 */

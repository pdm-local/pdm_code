...
export class ProjectAnalyzer {
	...
	private determineProjectType(
		...
		// A React app rendered with Ink is a terminal program, not a web app.
		// Checked before the web branch because Ink depends on React, so the
		// web detector matches first and every Ink CLI came out labelled
		// "React Web Application" - wrong, and fed to the model every turn.
		const rendersToTerminal = dependencies.frameworks.some(
			(f): f is Framework => (f as Framework).name.toLowerCase() === 'ink',
		);
		if (rendersToTerminal) {
			return 'Terminal Application (Ink)';
		}

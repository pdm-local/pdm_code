...
# NOTE: do NOT run `pnpm run build:credits`. History here is one squashed commit,
# so it would overwrite contributors.json - the only surviving record of who wrote
# the upstream code - with a single name.
...
# VS Code extension
pnpm run build:vscode   # Build extension to assets/pdm-vscode.vsix
```

## Project Overview

PDM Code is a React-based CLI coding agent built with Ink.js that provides local-first AI assistance with multiple provider support (Ollama, OpenRouter, any OpenAI-compatible API).
...
- `source/custom-commands/` - User-defined markdown commands from `.pdm/commands/`
...
- `source/daemon/` - Per-project daemon process: lockfile, Unix-socket IPC server / TUI client, CLI surface (`pdm daemon <start|stop|status|logs>`), launchd plist + systemd user unit installers
...
Two execution paths exist: native tool calling (preferred, via AI SDK) and an XML fallback for models that don't support tools. `LLMChatResponse.toolsDisabled` signals which path produced the response. The conversation loop runs `parseToolCalls()` (in `source/tool-calling/`) whenever the response has no native tool calls, always on the fallback path, and on the native path too, since models marketed as native-tool-capable sometimes regress to emitting tool-call text. Malformed text there feeds the self-correction retry loop capped by `pdm.retries.maxMalformedRetries`.
...
- **Single-file**: a `.md` in `.pdm/commands|agents|tools/`. Filename is the skill name. Optional `subscribe:` block in frontmatter declares event triggers. Backwards-compatible with the legacy flat dirs.
- **Bundle**: a directory under `.pdm/skills/<name>/` containing `skill.yaml` plus optional `commands/`, `agents/`, `tools/` subdirs. Multi-piece features (e.g. a `pr-reviewer` skill with a subagent + tool + command) live in a bundle.
...
`DaniMihai95 <DaniMihai95@users.noreply.github.com>` and by nobody else.

- **Never add a `Co-Authored-By:` trailer.** Not for Claude, not for any other AI
  assistant, not for a tool. An agent working here commits as the repository owner.
- **Never add "Generated with", "Made with", or any tool-attribution footer** to a
  commit message, a pull request body, or a changelog entry.
- **Never add a session or assistant link** (`Claude-Session:`, or similar) to a commit
  message.
- If a commit arrives with the wrong author, fix it before pushing:
  `git commit --amend --reset-author --no-edit`.

Write the message as a plain description of the change: a short imperative subject line,
then a body explaining *why* if the reason is not obvious from the diff.

This is about provenance, not about hiding tool use. The MIT attribution that PDM Code
genuinely owes - `LICENSE.md`, the Credits sections, and `contributors.json` - stays
exactly as it is and must never be stripped.

## Releasing

Nothing is published from here - no npm package, no Homebrew tap, no Nix package, no
GitHub release. `main` is the release. `.github/workflows/ci.yml` builds and runs the
test suite on every push and pull request; that check is the whole gate.

---
title: "AI Providers"
description: "Configure AI providers for PDM Code including Ollama, OpenRouter, and more"
sidebar_order: 2
---

# AI Provider Setup

PDM Code supports multiple AI providers including any OpenAI-compatible API, native Anthropic, Google Gemini, and GitHub Copilot through a unified provider configuration.

## Configuration Methods

1. **Interactive Setup (Recommended for new users)**: Run `/settings providers` inside PDM Code for a guided wizard with provider templates. The wizard allows you to:
...
|----------|-------------|
| `PDM_PROVIDERS` | JSON string containing provider configurations |
| `PDM_PROVIDERS_FILE` | Path to a JSON file (used if `PDM_PROVIDERS` is not set) |

The JSON value accepts a direct array, or the standard `agents.config.json` wrapper formats:

```bash
# Direct array
export PDM_PROVIDERS='[{"name":"my-provider","baseUrl":"http://localhost:1234/v1","models":["model-1"]}]'

# Wrapper format
export PDM_PROVIDERS='{"pdm":{"providers":[{"name":"my-provider","baseUrl":"http://localhost:1234/v1","models":["model-1"]}]}}'

# File-based
export PDM_PROVIDERS_FILE=/path/to/providers.json
```

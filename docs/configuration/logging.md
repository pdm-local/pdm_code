...
PDM Code includes structured logging with Pino, providing correlation tracking, performance monitoring, and automatic PII redaction.

## Quick Start

```bash
# Environment Variables
PDM_LOG_LEVEL=debug          # Log level (trace, debug, info, warn, error, fatal)
PDM_LOG_DIR=/var/log/pdm # Log directory override
PDM_CORRELATION_ENABLED=true  # Enable correlation tracking
```
...
- **macOS**: `~/Library/Logs/pdm`
- **Linux/Unix**: `~/.local/state/pdm/logs` (or `$XDG_STATE_HOME/pdm/logs`)
- **Windows**: `%LOCALAPPDATA%/pdm/logs`

You can override the default location using the `PDM_LOG_DIR` environment variable.

To disable file logging entirely, set `PDM_LOG_DISABLE_FILE=true`.
...
```bash
PDM_LOG_LEVEL=debug
PDM_CORRELATION_ENABLED=true
PDM_CORRELATION_DEBUG=true
```

**Production:**
```bash
PDM_LOG_LEVEL=info
PDM_LOG_DIR=/var/log/pdm
PDM_CORRELATION_ENABLED=true
PDM_CORRELATION_DEBUG=false
```
...
|----------|-------------|---------|
| `PDM_LOG_LEVEL` | Log level (trace, debug, info, warn, error, fatal) | `info` |
| `PDM_LOG_TO_FILE` | Enable file logging | `true` |
| `PDM_LOG_DIR` | Log directory override | Platform default |
| `PDM_LOG_DISABLE_FILE` | Disable file logging entirely | `false` |
| `PDM_CORRELATION_DEBUG` | Debug correlation tracking | `false` |
| `PDM_CORRELATION_ENABLED` | Enable correlation tracking | `true` |
...
- Check `PDM_LOG_LEVEL` allows your messages through (e.g. `debug` level won't show with `info` level set)
- Verify the log directory exists and is writable
- Check `PDM_LOG_DISABLE_FILE` is not set to `true`

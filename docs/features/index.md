---
title: "Features"
description: "A guided tour of PDM Code's features, from your first session to advanced workflows"
sidebar_order: 6
---

# Features

This guide walks you through PDM Code's features in the order you'll naturally discover them, starting with the basics you'll use every session, then building up to power-user workflows.

## Your First Session

When you launch PDM Code for the first time in a project directory, you'll see a security disclaimer asking you to trust the directory. Once confirmed, you're in an interactive chat session with your AI assistant.

Here's what you need to know right away:

### Talking to the AI

Type your message and press **Enter** to send. The AI streams its response token-by-token. If you need multi-line input, press **Ctrl+J** to add a new line; that is the officially supported newline shortcut.
...
Use **`@`** followed by a filename to include file contents in your message. PDM Code fuzzy-matches as you type and shows autocomplete suggestions; press **Tab** to select.
...
Prefix any command with **`!`** to run it directly in your shell without leaving PDM Code. The output becomes context for the AI.
...
For scripting and automation, run PDM Code without an interactive session:
...
Override the default mode with `--mode`. It works both interactively and with `run`:

```bash
pdm --mode yolo                      # interactive, no approvals
pdm --mode plan run "audit auth"     # run mode, plan only
```
For structured output that's easy to parse in scripts, add `--json`: `pdm --plain --json run "..."` prints a single JSON object to `stdout` with the final answer, tool call log, and modified files. See [Commands → JSON Output](commands.md#json-output).
...
PDM Code [automatically saves your sessions](session-management.md) so you can pick up where you left off:
...
```bash
pdm --continue    # or resume from your shell at launch
```
...
**[Skills](skills.md)** are the umbrella for everything you can plug into PDM Code: custom commands, subagents, custom tools, and event-driven triggers all live under one model. A skill is either a single `.md` file (a one-member skill) or a directory under `.pdm/skills/<name>/` (a bundle that ships a command + subagent + tools together).
...
Features include live diff previews of proposed changes, right-click "Ask PDM Code about this" for selected code, and LSP diagnostics sharing.

### ACP (Zed and other editors)

Run PDM Code as an [Agent Client Protocol server](acp.md) so ACP-compatible editors like Zed can drive it as a native agent, with diffs, tool cards, permission prompts, and model switching rendered in the editor:
...
Extend PDM Code's capabilities by connecting [MCP (Model Context Protocol) servers](../configuration/mcp-configuration.md). MCP servers add new tools the AI can use, from database queries to API calls to custom integrations.
...
| [Desktop Notifications](notifications.md) | Get notified when PDM Code needs your attention |

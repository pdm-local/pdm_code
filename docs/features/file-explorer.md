...
It also respects an optional `.pdmignore`, which uses the same pattern syntax and is for files that are tracked in git (and so aren't covered by `.gitignore`) but still aren't worth the context, like `package-lock.json` or generated fixtures. See [Ignoring Files](../configuration/index.md#ignoring-files).

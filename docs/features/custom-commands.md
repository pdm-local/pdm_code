...
mkdir -p .pdm/commands
```

`.pdm/commands/review-code.md`:
...
Commands live in `.pdm/commands/` in your project root. Each `.md` file becomes a command named after the file.
...
> files, restart PDM Code. (Bundle-skill commands declared in
> `.pdm/skills/<name>/commands/` also load at boot; there is no
> hot-reload.)

### `/commands create`

Creates a new command file in `.pdm/commands/` and starts an AI-assisted session to write its content. The AI will ask what you want the command to do, then write the markdown file for you.
